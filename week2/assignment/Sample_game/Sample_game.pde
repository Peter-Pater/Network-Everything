...
void setup() {
  ...
  myShip = new Ship(width/2 + 50, height - 100); // instantiate the ship
  myShip1 = new Ship(width/2 - 50, height - 100);
...
void draw() {
  ...
  if (myShip1.counter > 0 && myShip.counter > 0) {
    ...
    if (keyPressed) {
      myShip.counter = 100;
      myShip1.counter = 100;
      myShip.xPos = width/2 + 50;
      myShip.yPos = height/2 + 50;
      myShip1.xPos = width/2 - 50;
      myShip1.yPos = height/2 - 50;
      for (int x=0; x< starNum; x++) {
